...
{
    [TestFixture]
    public class InEarlyDataMapperTests
    {
        [Test]
        public void TestsMapping()
        {
            var mapper = new InEarlyDataMapper();
            ...
            Assert.AreEqual(exceptionRules[0].ExceptionRule.InEarly, result.Item2[0].Time);
            Assert.AreEqual(exceptionRules[1].ExceptionRule.InEarly, result.Item2[1].Time);
            Assert.AreEqual(exceptionRules[2].ExceptionRule.InEarly, result.Item2[2].Time);
            ...
            Assert.AreEqual(new TimeSpan(17, 0, 0), result.Item3[0].Time);
            Assert.AreEqual(new TimeSpan(17, 0, 0), result.Item3[1].Time);
            Assert.AreEqual(new TimeSpan(17, 0, 0), result.Item3[2].Time);
        }
    }
}
